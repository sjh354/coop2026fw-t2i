...
- [x] **CSD golden set 보강(lecture24 8카테고리)**: `refs/lecture24/<카테고리>/`에 있던 후보 이미지(카테고리당 15~24장, 총 158장)를 `scripts/validate_ref_set.py`로 검증(8개 전부 `validated`, outlier 없음, 매니페스트 `configs/ref_sets/*.yaml`). 카테고리마다 다른 ref_set을 써야 해서 `src/scoring.py --components csd`(단일 ref_set 전제) 대신 다중참조 평균 채점 전용 `scripts/score_csd_golden.py`를 새로 작성, 기존 csd_target 채점이 있던 18개 run·420장을 재채점. 상관관계는 강하지만(Pearson r=0.83) 절대값은 80% 이미지에서 하락하고 카테고리별 방향도 갈린다 — 상세는 `bench/results.md`의 "lecture24 CSD golden set 보강" 절과 `reports/csd-golden-set-lecture24/index.html`.
