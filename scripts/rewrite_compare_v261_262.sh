#!/usr/bin/env bash
# TASK-E 후속: 개량된 시스템 프롬프트(wan_style_cn=v261 / promptenhancer_cn=v262,
# flux2-klein-4b-nf4) 생성 결과에 네 가지 지표를 전부 채점한다. v250_252 스크립트와 동일
# 패턴 — v251(wan_style)/v252(promptenhancer)를 대체하는 baseline 후보 비교용.
# 전부 서버 23(채점 전용)에서 실행: vqascore+custom_cv(t2i-score, src.scoring),
# csd_target(t2i-score, PYTHONPATH=vendor), vlm-judge(t2i-judge2, InternVL3-8B 기본값,
# scripts/judge_lecture24).
#
#     bash scripts/rewrite_compare_v261_262.sh
#
# 산출물 (버전별 bench/scores/<run>/ 아래):
#   pass1.csv, csd_target.csv, judge_lecture24.csv
# 합본:
#   bench/scores/rewrite_v261_262_pass1.csv
#   bench/scores/rewrite_v261_262_csd_target.csv
#   bench/scores/rewrite_v261_262_judge_lecture24.csv
set -u
...
TASK="rewrite_compare_v261_262.sh"
LOG_DIR="logs/$(date +%Y%m%d_%H%M%S)_rewrite_compare_v261_262"
mkdir -p "$LOG_DIR"
ALERT="$(dirname "$0")/alert.py"

RUN_DIRS=(
  "v261_flux2-klein-4b-nf4-lecture24"
  "v262_flux2-klein-4b-nf4-lecture24"
)
declare -A BACKEND=(
  ["v261_flux2-klein-4b-nf4-lecture24"]="wan_style_cn"
  ["v262_flux2-klein-4b-nf4-lecture24"]="promptenhancer_cn"
)
...
echo "=== 합본 CSV 생성"
merge_csv bench/scores/rewrite_v261_262_pass1.csv "${pass1_parts[@]}" \
  && echo "    -> bench/scores/rewrite_v261_262_pass1.csv"
merge_csv bench/scores/rewrite_v261_262_csd_target.csv "${csd_parts[@]}" \
  && echo "    -> bench/scores/rewrite_v261_262_csd_target.csv"
merge_csv bench/scores/rewrite_v261_262_judge_lecture24.csv "${judge_parts[@]}" \
  && echo "    -> bench/scores/rewrite_v261_262_judge_lecture24.csv"

echo
echo "로그: $LOG_DIR/"
conda run -n t2i-score python "$ALERT" --task "$TASK" --status $([ "$fail" -eq 0 ] && echo ok || echo fail) \
  --message "TASK-E 개량 시스템 프롬프트 2조건(wan_style_cn/promptenhancer_cn) 4지표 채점 완료 — 로그: $LOG_DIR"
exit "$fail"
