...
# bias) 여부를 확인한다. 172.10.5.23(채점 전용 서버, conda env t2i-judge)에서
# 실행. unsloth/gemma-3-12b-it-bnb-4bit(사전 4bit 양자화, ~7.3GB, ungated)를
# 쓴다 — google/gemma-3-12b-it 원본(22.7GB, gated)은 서버 여유 디스크(~16GB)에
# 안 들어가서 내려온 선택.
#
# 산출물:
...
if conda run -n "$ENV" python -m scripts.judge_agreement \
  ...
  echo "    OK"
  cat "$AGREEMENT_LOG"
  python "$(dirname "$0")/../alert.py" --task "$TASK" --status ok --log "$AGREEMENT_LOG"
else
