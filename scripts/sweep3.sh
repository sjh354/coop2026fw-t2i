#!/usr/bin/env bash
# sweep2에서 안 돌린 4개 실험(coloring-book-es, flat-illust-es, educational-flat,
# educational-flat-es) + 이번에 새로 추가한 diagram/formula/history 6개 세트,
# 총 10개 실험 × 5모델을 돈다.
#
#   bash scripts/sweep3.sh
#
# pixart-sigma는 sweep2에서 캐시가 없는 상태로 맨 앞에 있어 SKIP_DISK로
# 건너뛰어졌다. sweep3에서는 pixart-sigma를 맨 뒤로 돌려, 앞선 4개 모델이
# DONE_MODELS에 쌓인 뒤 reclaim()이 그 캐시를 지워 공간을 확보하고
# pixart-sigma를 재다운로드하도록 한다. 구조는 sweep2.sh와 동일
# (모델 루프가 바깥, 디스크 회수 로직 동일).

set -u

MIN_FREE_GB=20          # 다음 모델을 받기 전 확보돼야 할 최소 여유
LOG_DIR="logs/$(date +%Y%m%d_%H%M%S)_sweep3"
...
python "$(dirname "$0")/alert.py" --task "sweep3.sh" --status "$status" --log "$SUMMARY"
