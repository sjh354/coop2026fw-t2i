#!/usr/bin/env bash
# 서버 1: ideogram-4 / sana-1600m / sd35-medium 담당.
# (서버 2는 sweep9_2.sh 가 qwen-image / flux2-klein-9b / lumina2 담당)
# 대략 감만 잡는 용도라 -es 실험은 빼고, formula-*/history-* 는 각각 하나씩만 돈다.
#
# 분배 근거 — 지금까지 sweep 로그의 sec/img 실측치 * 30장 * 남은 실험 수:
#   ideogram-4     ~162s/img (educational-flat 포함 5개 실험) -> ~6.8h
#   sana-1600m     ~3.8s/img  (4개 실험)                       -> ~0.1h
#   sd35-medium    ~13.7s/img (4개 실험)                       -> ~0.5h
#   합계 서버1 ≈ 7.4h   (서버2: qwen-image ~5.2h + flux2-klein-9b ~0.9h + lumina2 ~1.0h ≈ 7.1h)

set -u

MIN_FREE_GB=35          # ideogram-4/qwen 계열 gguf+encoder 합쳐 ~30GB대라 여유 크게
LOG_DIR="logs/$(date +%Y%m%d_%H%M%S)_sweep8_1"
...
# ideogram-4는 educational-flat 마지막 시도가 running에서 멈춰있어서 재시도 목록에 포함.
EXPS_IDEOGRAM=(
  educational-flat
  "${EXPS_STD[@]}"
)
...
# 조합 하나 끝날 때마다 디스코드로 알림 (예: "SUCCESS : ideogram-4/diagram-clean/diagram30").
notify_attempt() {
