...
>
> **재검토(라벨 오염 발견 및 수정)**: claude/qwen s8 재채점 질문에서 "yes" 선택지를 먼저 제시하고
> 설명을 붙이는 방식으로 질문해, 사람이 유도됐을 가능성이 발견됨(원래 사람은 두 이미지 모두 s8에
> "no"라고 답했었는데 재질문 후 "yes"로 뒤집힘 — 유도 편향 의심). 추가로 s8이 "thick and black"을
> 하나의 문항에 묶은 것 자체가 원자성 위반(어느 조건이 실패인지 구분 불가)이었음을 확인.
> → s8을 색상(흑색 여부)만 남기고, 두께는 새 항목 s10으로 분리. 두 항목 모두 옵션 순서를
> 중립화하고 유도성 설명을 뺀 재질문으로 다시 확인 → **claude/qwen 모두 s8=yes, s10=yes로 재확인**
> (동일 결과 재현, 유도 편향 아님으로 판단). server 23에서 judge_spec.py 재실행해 auto 쪽도
> s8/s10 갱신.
>
> **최종 일치율(29건, s10 추가로 26→29항목)**: **22/29 = 0.76**. 여전히 0.8 미만이지만 B 게이트는
> 완전히 해소됨. 남은 불일치는 chatgpt s8(배경)/s9(간격) 뿐이며 원인은 spec 문구가 아니라
> chatgpt 소스 이미지 자체의 생성 실패(의도한 worksheet 구조가 거의 안 보임)로 판단 — 더 이상
> 문구를 수정하지 않고 STAGE 1로 진행.
