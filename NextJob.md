...
```
[배경]
PPT의 "3종 프롬프트 소스 비교"(TASK-E, passthrough/wan_style/promptenhancer, 8-9p 슬라이드)는
소스를 통째로 바꿔서 비교했다 — wan_style/promptenhancer는 passthrough 대비 "내용(어휘 선택)도
다르고 길이도 길다"가 같이 바뀐 상태라, 점수 차이가 리라이팅 품질 때문인지 단순히 프롬프트가
길어져서인지 분리가 안 된다. **이 태스크는 두 파트로 나뉜다 — 서로 다른 질문에 답한다.**

[파트 1 — 재사용 분석: "리라이팅이 카테고리별로 다르게 작동하는가" (새 생성 불필요)]
passthrough(v250, 원본)와 wan_style_cn(v261)/promptenhancer_cn(v262, 리라이팅)는 **같은 24개
lecture-vlm24 프롬프트**에 대해 model/steps/guidance/seed 전부 동일(직접 대조 확인 완료,
`image-prompts/v25{0}_*/*.md` vs `v26{1,2}_*/*.md` 프론트매터)하게 리라이팅만 다르게 돌렸고,
이미 4지표 채점까지 끝나 있다(`bench/scores/rewrite_v250_252_*.csv`, `rewrite_v261_262_*.csv`).
**주의: 이건 "길이 효과"가 아니라 "리라이팅(내용+길이 동시 변화) 효과"다** — 파트 1은 어휘가
같이 바뀌는 걸 인정한 채로 그냥 "카테고리별로 리라이팅이 다르게 작동하는가"만 본다. 길이를
분리해서 보는 건 파트 2다.
- 카테고리별 counting/attribute-binding 여부는 눈대중이 아니라 `configs/benchmarks/vlm-prompts.json`
  각 항목의 `metric` 배열 문자열(예: "6 boxes", "1:1 box-to-pin connection", "color-order attribute
  binding")에 counting/binding 키워드가 있는지로 기계적으로 분류한다.
- 1. `scripts/analyze_rewrite_by_category.py` 신규 — 두 채점 CSV 세트(v250_252 / v261_262)를
  item_id 기준 조인 + 위 방식으로 카테고리 분류.
- 2. "counting/binding 카테고리 vs 나머지"를 별도로 두 번 유의성 검정하고 "한쪽만 유의하다"고
  결론 내지 마라 — **그 자체가 오류다.** 대신 그룹 간 delta 차이를 직접 검정한다: 항목별
  delta(리라이팅 - passthrough) 계산 후, 두 그룹의 delta 분포를 Mann-Whitney U 또는 paired
  bootstrap(그룹 평균 delta 차이의 CI)로 직접 비교. n은 그룹당 4카테고리×3소스=12개다(8~4개가
  아님 — 소스 3개가 counting 그룹 안에도 다 들어있어서 12개).

[파트 2 — 신규 실험: 길이만 분리한 사다리(ladder), 어휘 계열 고정]
파트 1로는 길이 자체의 효과를 답할 수 없다(내용도 같이 바뀌므로). 길이만 분리하려면 **하나의
리라이팅 계열(예: promptenhancer_cn 결과)을 골라, 그 출력을 다른 단어 예산으로 잘라낸 버전들을
만들어 같은 어휘 소스에서 길이만 다른 조건을 비교**한다.
- 단어 예산 3단계 제안: ~75(원본 claude 소스 평균과 비슷) / ~150 / ~300(promptenhancer_cn 원문).
  README:28의 스타일 프리셋 배열 규칙(스타일 앵커→제약→배경→구도→금지, 뒤가 잘려도 앞이
  살아남게)과 동일하게, **앞에서부터 살리고 뒤를 자른다** — 즉 이 사다리는 순수 토큰 개수가
  아니라 "얼마나 많은 제약/디테일을 뒤에서부터 덜어내는가"를 측정한다. 진짜 순수 길이(같은
  정보량, 단어 수만 다름)는 이 파이프라인으로는 만들 수 없다 — 이 한계를 슬라이드에 그대로
  명시할 것.
- 24개 전부가 아니라 counting/attribute-binding 카테고리 4개(위 분류 기준)만 대상으로 한다
  (12개 프롬프트 = 4카테고리 × 3소스). 나머지는 이 사다리 실험에서 제외 — 초점을 좁힌다.
- 생성: 157 서버, flux2-klein-4b-nf4, model/steps/guidance/seed는 v250/v261/v262와 동일하게
  고정(비교 가능성 유지). `scripts/alert.py`로 완료/실패 알림 필수(모델 추론이므로).
- 채점: 기존 4지표 파이프라인(`scripts/rewrite_compare_*.sh` 패턴 재사용) 그대로.
- 분석: x=단어수(3단계), y=지표값, Wilcoxon 대신 단조 추세 검정(Jonckheere-Trend 또는 최소한
  Spearman 상관 + paired bootstrap CI) — VQAScore가 유독 이 사다리에서 하락하는지가 핵심
  관심사(스터디 노트 §2-2 "VQAScore는 긴 프롬프트에서 한계"를 직접 검증).

[해야 할 일 순서]
1. 파트 1(재사용, 생성 없음) 먼저 끝내고 결과를 본다.
2. 파트 1 결과가 이미 "counting 그룹만 리라이팅에 취약하다"를 명확히 보여주면, 파트 2는
   "그 취약성이 순수 길이 때문인지 디테일 손실 때문인지"를 좁혀서 검증하는 후속으로 진행.
   파트 1이 애매하면 파트 2 없이 결론을 보류하고 사용자에게 먼저 보고한다 — 곧바로 파트 2
   생성에 들어가지 않는다.

[하지 말 것]
- passthrough/wan_style_cn/promptenhancer_cn 3조건을 다시 생성하지 마라 — 이미 있다(파트 1).
- 파트 1과 파트 2의 결론을 섞어서 "길이 때문"이라고 뭉뚱그리지 마라 — 파트 1은 리라이팅 효과,
  파트 2만 길이 효과다.
- 통계적으로 유의하지 않은데 "경향이 있어 보인다"는 식으로 슬라이드에 넣지 마라.
```

---

### TASK-J · 스타일 정렬 임계값 → LoRA 전환 의사결정 트리

```
[배경]
지금 스타일 정렬은 전부 "프롬프트에 스타일 앵커 문구를 넣는" prompt-level 방식이고, LoRA
파인튜닝은 아직 손대지 않았다(README 체크리스트에 "CSD golden set 보강"만 있고 "정렬이 안 될
때 뭘 한다"는 기준이 없음). 이 실험은 prompt-level 정렬만으로 프리셋별 CSD가 어느 수준까지
나오는지 먼저 측정하고, 그 아래로 떨어지면 LoRA로 넘어간다는 임계값/의사결정 기준을 세운다.

[중요한 사전 지식 — 재사용 우선]
`bench/scores/merged.csv`에 이미 필요한 데이터가 있다. bench_v1 12개 run(v231~v242 =
4개 프리셋 × 3개 후보 모델, `edu-flat-v2`/`observational`/`playful-soft`/`storybook-scene`)이
**검증된 golden set**(README §114, Imagen-4로 독립 생성, `validate_ref_set.py` 통과) 대비 csd
컬럼으로 이미 채점돼 있다 — 이게 "prompt-level 정렬만으로의 CSD" 실측치다. **새로 생성할
필요 없이 merged.csv를 프리셋별로 재집계하면 1차 결과가 나온다.**
주의: 이건 lecture24의 `csd_target`(카테고리당 참조 1장, provisional, `bench/results.md`
§14)과 다른 컬럼(`csd`)이다 — bench_v1의 `csd`가 golden set 기반 정식 지표이니 이걸 쓴다.

[해야 할 일]
1. `merged.csv`에서 run→preset 매핑 복원(`image-prompts/v23{1..42}_*/*.md` 프론트매터의
   `exp:` 필드로 조인 — bench_v1이 어떤 experiment=preset으로 돌았는지 확인).
2. 프리셋별 csd 평균 + 95% CI(paired bootstrap, 스터디노트 §2-1 방법 재사용) 산출. 3개
   모델을 합쳐서 볼지 모델별로 따로 볼지는 먼저 분산을 보고 정한다 — 모델 간 분산이 크면
   합치지 말고 모델×프리셋 둘 다 보고한다.
3. **임계값 정의가 이 태스크의 핵심 미해결 지점이다.** 현재 절대적인 "CSD 몇 점이면 합격"
   기준이 프로젝트에 없다. 후보 두 방식 중 검토 후 하나 선택(또는 병기):
   (a) golden set 내부 self-similarity를 CSD로 재보고(같은 프리셋 참조 이미지끼리 pairwise
       CSD) 이걸 "이론적 상한"으로 삼아, 생성 이미지의 CSD가 그 상한의 몇 %인지로 정규화.
       예: 상한 대비 70% 미만인 프리셋 → LoRA 후보. **상한만으로는 "70%"란 숫자가 해석 불가능
       하다 — 하한(무정렬 기준선)도 같이 재야 한다**: 예를 들어 프리셋 A의 골든셋과 무관한
       프리셋(B) 골든셋 간 CSD, 또는 lecture24 사진풍 카테고리처럼 스타일이 전혀 안 맞는
       이미지 간 CSD를 "정렬 안 됐을 때 기저값"으로 같이 보고해서 상한-하한 사이 어디쯤인지로
       해석한다.
   (b) 4개 프리셋 중 상대적으로 가장 낮은 프리셋을 기준점 삼아 "이보다 유의하게 낮으면"으로
       상대 판정(절대 임계값 대신 프리셋 간 순위 기반).
   **이 중 어느 쪽으로 갈지, 혹은 다른 기준이 있는지는 결정 전에 나한테 먼저 물어봐라** —
   임의로 숫자(예: "0.5 미만")를 하드코딩해서 확정하지 마라. 근거 없는 임계값은 발표에서
   바로 찔린다.
4. 의사결정 트리 정리: "프리셋 확정 → prompt-level CSD 측정 → 임계값 이상이면 채택 → 미만이면
   LoRA 데이터셋 준비 착수"를 다이어그램 1개로. `bench/results.md`에 "스타일 정렬 임계값" 절
   신규, README 체크리스트에도 반영.

[검증 기준]
- 프리셋별 n(=3모델×10~15키워드)이 작다 — CI가 넓으면 "임계값 하회"를 단정하지 말고 "더 봐야
  한다"로 보고한다.
- LoRA로 넘어가자는 결론이 나와도 이번 태스크에서 LoRA를 실제로 시작하지 않는다 — 기준선을
  세우는 것까지가 범위. 다음 세션 결정 사항으로 넘긴다.

[하지 말 것]
- bench_v1 12개 run을 재생성하지 마라 — 이미 golden set 대비 채점 완료돼 있다.
- lecture24의 `csd_target`(provisional, 참조 1장)을 이 임계값 산출에 쓰지 마라 — bench_v1의
  `csd`(golden set 기반)만 쓴다. 둘을 섞으면 지표 정의가 바뀌는 걸 숨기게 된다.
- 임계값 숫자를 이 세션에서 혼자 확정하지 마라 — (a)/(b) 방식 선택과 실제 컷오프 값은 반드시
  사용자 확인 후 문서에 반영한다.
```
